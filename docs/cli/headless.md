...
```bash
gemini-uk --prompt "Що таке машинне навчання?"
```
...
```bash
cat README.md | gemini-uk --prompt "Зроби підсумок цієї документації"
```
...
```bash
gemini-uk -p "Яка столиця Франції?"
```
...
```bash
gemini-uk -p "Яка столиця Франції?" --output-format json
```
...
```bash
# Зберегти у файл
gemini-uk -p "Поясни Docker" > docker-explanation.txt

# Передати в jq
gemini-uk -p "Що таке Kubernetes?" --output-format json | jq '.response'
```
...
| Опція                   | Опис                           | Приклад                                          |
| ----------------------- | ------------------------------ | ------------------------------------------------ |
| `--prompt`, `-p`        | Запуск у безголовому режимі    | `gemini-uk -p "запит"`                           |
| `--output-format`       | Формат виводу (text, json)     | `gemini-uk -p "запит" --output-format json`      |
| `--model`, `-m`         | Вказати модель Gemini          | `gemini-uk -p "запит" -m gemini-2.5-flash`       |
| `--debug`, `-d`         | Увімкнути режим налагодження   | `gemini-uk -p "запит" --debug`                   |
| `--include-directories` | Включити додаткові каталоги    | `gemini-uk -p "запит" --include-directories src` |
| `--yolo`, `-y`          | Автоматично схвалювати всі дії | `gemini-uk -p "запит" --yolo`                    |
...
```bash
cat src/auth.py | gemini-uk -p "Перевір цей код на проблеми з безпекою" > security-review.txt
```

#### Генерація повідомлень коміту

```bash
result=$(git diff --cached | gemini-uk -p "Напиши лаконічне повідомлення коміту" --output-format json)
