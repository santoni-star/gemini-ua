...
- **Інструменти для збірки:** Для компіляції деяких нативних залежностей
  (наприклад, `node-pty`) можуть знадобитися системні інструменти для збірки:
  - **Для Windows:** Запустіть `npm install --global windows-build-tools` у
    терміналі адміністратора.
  - **Для macOS:** Встановіть інструменти командного рядка Xcode, виконавши
    `xcode-select --install`.
  - **Для Debian/Ubuntu:** Виконайте `sudo apt-get install -y python3 make g++`.
