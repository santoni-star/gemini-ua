...
export function EditorSettingsDialog({
  ...
  if (
    ...
    otherScopeModifiedMessage =
      settings.forScope(selectedScope).settings.general?.preferredEditor !==
      undefined
        ? strings.editorAlsoModifiedIn.replace('{scope}', otherScopeLabel)
        : strings.editorModifiedIn.replace('{scope}', otherScopeLabel);
  }
    ...
    >
      <Box flexDirection="column" width="45%" paddingRight={2}>
        <Text bold={focusedSection === 'editor'}>
          {focusedSection === 'editor' ? '> ' : '  '}{strings.editorSelect}{' '}
        ...
        <Box marginTop={1} flexDirection="column">
          <Text bold={focusedSection === 'scope'}>
            {focusedSection === 'scope' ? '> ' : '  '}{strings.editorApplyTo}
          </Text>
          <RadioButtonSelect
            items={scopeItems}
            initialIndex={0}
