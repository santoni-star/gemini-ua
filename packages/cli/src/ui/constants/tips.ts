...
export const INFORMATIVE_TIPS = [
  ...
  'Встановіть улюблений редактор для відкриття файлів (/settings)…',
  'Увімкніть режим Vim для модального редагування (/settings)…',
  'Вимкніть автоматичні оновлення, якщо хочете керувати вручну (/settings)…',
  'Вимкніть набридливі сповіщення про оновлення (settings.json)…',
  'Увімкніть чекпоінти для відновлення сесії після збою (settings.json)…',
  'Змініть формат виводу на JSON для скриптів (/settings)…',
  'Персоналізуйте CLI за допомогою нової кольорової теми (/settings)…',
  'Створюйте та використовуйте власні теми (settings.json)…',
  'Сховайте заголовок вікна для мінімалістичного вигляду (/settings)…',
  "Не подобаються ці поради? Ви можете їх приховати (/settings)…",
